...
namespace Bonsai.VideoAnnotations
{
    [DefaultProperty("FileName")]
    [Description("Produces a video sequence of images from the specified movie file.")]
    public class VideoNavigator : Source<ElementIndex<IplImage>>
    {
        int currentPosition;
        int? targetPosition;
        ...
        [FileNameFilter("Video Files|*.avi;*.wmv;*.mov;*.mp4;*.mpeg4;*.mkv;*.ogg|AVI Files (*.avi)|*.avi|All files (*.*)|*.*")]
        ...
        {
            targetPosition = Math.Max(0, Math.Min(frameNumber, frameCount - 1));
        }
        ...
        IEnumerable<ElementIndex<IplImage>> CreateCapture()
        {
            ...
            frameCount = (int)capture.GetProperty(CaptureProperty.FrameCount);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    currentPosition = (int)capture.GetProperty(CaptureProperty.PosFrames) - 1;
                    yield return new ElementIndex<IplImage>(image, currentPosition);
                }
            }
            finally
            {
        ...
        public override IObservable<ElementIndex<IplImage>> Generate()
        {
            return Observable.Create<ElementIndex<IplImage>>((observer, cancellationToken) =>
            {
        ...
        public IObservable<ElementIndex<IplImage>> Generate(IObservable<int> source)
        {
            var seek = source.Do(delta => Seek(currentPosition + delta));
            return Observable.Merge(
                Generate(),
                seek.IgnoreElements().Select(x => default(ElementIndex<IplImage>)));
        }
    }
